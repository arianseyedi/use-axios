 ...
 * @author Arian Seyedi
 */
function useAxios(config, transformer, axiosPromiseConstructor, options) {
    var _this = this;
    var _a = react_1.useState(defaultState), state = _a[0], setState = _a[1]; // simple one time fetch (either - or basis)
    var _b = react_1.useState(0), redo = _b[0], setRedo = _b[1];
        ...
        var loadData = function () { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        if (pollingInterval)
                            setIntervalState({ error: false, loading: true });
                        else
                            setState({ error: false, loading: true });
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, axiosPromiseConstructor(config, source.token)
                    ...
                    case 2:
                        ...
                        if (pollingInterval)
                            ...
                            setState({ error: false, loading: false, data: transformer(response.data) });
                        return [3 /*break*/, 4];
                    case 3:
                        error_1 = _a.sent();
                        if (axios_1.default.isCancel(error_1)) {
                            console.warn('FetchError - is cancel', error_1);
                            // do something when axios is cancelled
                        }
                        else {
                            console.log('FetchError: , ', error_1);
                            if (pollingInterval)
                                setIntervalState({ error: true, loading: false });
                            else
    ...
    return intervalState.data
        ...
        : state.data
            ...
            : [
                { data: undefined, error: state.error, loading: state.loading },
                { redo: handleIncrementRedo },
            ];
}
